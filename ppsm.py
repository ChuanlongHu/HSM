...
def process_non_tree_query(query_graph, data_subgraph_map, data_block_cut_tree, tutte_index, k, data_bridges):
    ...
    subgraph_map_q_list, graph_poly_q = tutte_from_bridge_decomposition(cut_edges_q, subgraphs_q)
...
def validate_and_find_matches(
    ...
    # 启动算法
    backtrack(0, {}, set(), 0, {})
    return verified_subgraphs, stats['search_count']


import networkx as nx

def validate_and_find_matches_baseline(
    query_graph: nx.Graph,
    k: int,
    candidate_vertex_map: dict,
    final_path_dict: dict
    ...
    """
    朴素基线算法（Ablation Baseline）：
    1. 不采用紧耦合即时路由（解耦：先全部映射顶点，再统一寻找路径）。
    2. 不采用预算前瞻预测（盲目搜索：只有当真实超标时才回溯）。
    返回: (结果列表, 搜索次数)
    """
    verified_subgraphs = []
    num_query_edges = query_graph.number_of_edges()
    ...
    # 统计搜索次数的计数器（使用字典以方便在嵌套函数中修改）
    stats = {'search_count': 0}
    
    ordered_nodes = sorted(query_graph.nodes(), key=lambda n: (-query_graph.degree(n), len(candidate_vertex_map.get(n, []))))
    
    original_edges = list(query_graph.edges())
    edge_directions = {tuple(sorted((u, v))): (u, v) for u, v in original_edges}

    # ================= 阶段 2：盲目路径路由 =================
    def route_all_edges(edge_idx, mapping, used_d_nodes, current_weight, edge_paths):
        stats['search_count'] += 1  # 记录边路由的搜索次数

        if edge_idx == len(original_edges):
            res_graph = nx.Graph()
            for path in edge_paths.values():
                nx.add_path(res_graph, path)
            ...
            for q_edge_key, path in edge_paths.items():
                if len(path) > 2:
                    ...
                    if path[0] == mapping[u]:
                        ...
                        internal = path[-2:0:-1]
                    for v_idx, d_node in enumerate(internal):
                        full_map[f"v_{u}_{v}_{v_idx}"] = d_node
                        
            res_graph.graph['mapping'] = full_map
            verified_subgraphs.append(res_graph)
            return

        q_u, q_v = original_edges[edge_idx]
        d_pair = tuple(sorted((mapping[q_u], mapping[q_v])))
        possible_paths = final_path_dict.get(d_pair, [])

        for path, p_len in possible_paths:
            # 【退化】没有任何前瞻预测，仅判断当前消耗是否直接爆表
            if current_weight + p_len > total_max_weight:
                continue 

            internal_nodes = set(path[1:-1])
            if internal_nodes.isdisjoint(used_d_nodes):
                edge_paths[tuple(sorted((q_u, q_v)))] = path
                used_d_nodes.update(internal_nodes)
                
                route_all_edges(edge_idx + 1, mapping, used_d_nodes, current_weight + p_len, edge_paths)
                
                used_d_nodes.difference_update(internal_nodes)
                del edge_paths[tuple(sorted((q_u, q_v)))]

    # ================= 阶段 1：顶点骨架寻找 =================
    def map_vertices(node_idx, mapping, used_d_nodes):
        stats['search_count'] += 1  # 记录顶点映射的搜索次数

        if node_idx == len(ordered_nodes):
            # 顶点全部映射完毕，触发第二阶段
            route_all_edges(0, mapping, used_d_nodes.copy(), 0, {})
            return
        ...
        for d_u in candidate_vertex_map.get(q_u, []):
            ...
            # 【退化】仅做最基础的连通性检查，不在这里连线，也不算权重
            is_feasible = True
            for q_v in matched_neighbors:
                d_v = mapping[q_v]
                d_pair = tuple(sorted((d_u, d_v)))
                if not final_path_dict.get(d_pair):
                    is_feasible = False
                    break
            ...
            if is_feasible:
                ...
                map_vertices(node_idx + 1, mapping, used_d_nodes)
...
def ppsm(query_graph: nx.Graph,
    ...
    t0= time.time()
    subgraph_map, total_tutte = tutte_from_bridge_decomposition(bridges, components)
    t1= time.time()
    print("tutte time:",t1-t0)
    ...
    #通过多项式覆盖初步估计是否存在解
    if maybe_contains_by_rect(q_tutte,total_tutte):
        #查询处理
        candidate_set=dispatch_query_processing(query_graph, subgraph_map, block_cut_tree, tutte_index,k,bridges)
        #构建候选解空间
        candidate_vertex_map, candidate_edge_set, candidate_path_dict = construct_candidate_solution_space(query_graph,k,candidate_set,subgraph_map,bridges)
        print("cs size:", sum(len(cands) for cands in candidate_vertex_map.values()))
        print("e size:", sum(len(v) for v in candidate_edge_set.values()))
        print("p size:", sum(len(v) for v in candidate_path_dict.values()))
        #过滤策略
        ...
        print("p filter:", sum(len(v) for v in candidate_path_dict.values())- sum(len(v) for v in f_path_dict.values()))
        #验证与回溯
        matchs, search_count = validate_and_find_matches(query_graph,k,f_candidate_vertex_map,f_path_dict)
        return matchs, search_count
    else:
        print("没有匹配")
        return []
...
def main():
    query_graph = nx.Graph([(0,1), (0,2),(1,2), (1,3), (2,3)])
    file_path = "./bio-CE-HT.txt" 
    data_graph = load_graph_from_txt(file_path)
    t0= time.time()
    matches, search_count = ppsm(query_graph,3,data_graph)
    t1= time.time()
