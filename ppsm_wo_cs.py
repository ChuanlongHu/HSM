...
def construct_candidate_solution_space_naive(
    query_graph: nx.Graph,
    k: int,
    candidate_subgraph_ids: set,
    data_subgraph_map: list,
    ...
    data_subgraph_dict = {
        sg_id: sg_graph
        for sg_id, sg_graph, _ in data_subgraph_map
    }
    ...
    # 2. 候选顶点集合（仅度约束）
    # ==========================================================
    q_degree = dict(query_graph.degree())
    ...
    for q_u, q_v in query_graph.edges():
        ...
        for d_u, d_v in product(cv_map[q_u], cv_map[q_v]):
            ...
            try:
                # 仅最短路径约束
                dist = nx.shortest_path_length(cdg, d_u, d_v)
            ...
            # 提取所有 simple paths
            # --------------------------------------------------
            if d_key not in candidate_path_dict:
...
def ppsm(query_graph: nx.Graph,
    ...
    if maybe_contains_by_rect(q_tutte,total_tutte):
        ...
        #构建候选解空间
        candidate_vertex_map, candidate_edge_set, candidate_path_dict = construct_candidate_solution_space_naive(query_graph,k,candidate_set,subgraph_map,bridges)
...
def main():
    ...
    file_path = "./graphs/ws_Et50_Er112_n56.txt" 
    data_graph = load_graph_from_txt(file_path)
    t0= time.time()
    matches = ppsm(query_graph,3,data_graph)
    t1= time.time()
